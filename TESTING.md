...
Manual offline check: install dependencies while online, run `npm run studio`, disconnect from the internet, and open `http://127.0.0.1:4333`. Choose **Work on local drafts**, select a poster and video, enter an incomplete product, and click **Save on this PC**. Close and reopen the browser using the same profile and address. Open the saved draft and confirm the text, local poster preview, video filename, and store links are intact. After reconnecting, sign in with an approved editor and explicitly save/publish. Nothing should upload merely because the connection returned.
