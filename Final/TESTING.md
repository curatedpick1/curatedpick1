...
The minimal Studio interface was checked in a visible Chromium browser for collapsed optional fields, one-time account/connection settings, password-only unlocking after reload, rejection of a wrong password (mocked Auth), remembered board ID, offline drafts, and mobile layout. The Windows launcher was executed and verified to start/reuse a background server and open the browser. The Linux launcher passed Bash syntax checking; its file-manager behavior was not tested on a Linux desktop. The launcher requires Node.js 24 and installs missing project dependencies on first launch.
